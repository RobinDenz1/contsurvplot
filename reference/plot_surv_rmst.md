...
restricted mean survival time (RMST) as a function of a continuous
variable.
...
- tau:
  ...
  The point in time to which the RMST should be calculated. Can be a
...
Similar to the
[`plot_surv_at_t`](https://robindenz1.github.io/contsurvplot/reference/plot_surv_at_t.md)
and
[`plot_surv_quantiles`](https://robindenz1.github.io/contsurvplot/reference/plot_surv_quantiles.md)
plots, this function produces a plot of a survival curve summary
statistic as a function of a continuous variable. The summary statistic
in question is the restricted mean survival time (RMST), which is the
area under a survival curve up to a specific point in time `tau`. It can
be interpreted as the mean survival time of the population up to `tau`.

First, a range of value-specific survival curves are estimated. The RMST
is then calculated for each one using step function interpolation. This
only works for survival curves, not for CIFs. If the area under the CIF
should be used instead, the
[`plot_surv_rmtl`](https://robindenz1.github.io/contsurvplot/reference/plot_surv_rmtl.md)
function can be used.

An advantage of this method over the
[`plot_surv_at_t`](https://robindenz1.github.io/contsurvplot/reference/plot_surv_at_t.md)
and
[`plot_surv_quantiles`](https://robindenz1.github.io/contsurvplot/reference/plot_surv_quantiles.md)
function is, that it does take the whole survival curve into account,
kind of. Although it is the area under it, it is only calculated up to
`tau` which makes the output dependent on the choice of `tau`. This,
again, can result in deceiving plots in some cases. Plots visualizing
the entire survival curves such as
