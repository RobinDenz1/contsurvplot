...
test_that("plot, defaults", {
  plt <- plot_surv_tiles(time="time", status="event", variable="x3",
...
test_that("plot, change n_col and n_row", {
  plt <- plot_surv_tiles(time="time", status="event", variable="x3",
                         data=sim_dat, model=model, nrow=5, ncol=5)
  expect_s3_class(plt, "ggplot")
...
test_that("n_col too big", {
  expect_error(plot_surv_tiles(time="time", status="event", variable="x3",
